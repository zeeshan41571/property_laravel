        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <a href="{{ route('parkings.create') }}" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
                    <br><br>
                    @if (session('success'))
                    ...
                    <table width='100%' class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="border text-left" style="text-align: left;">Arrival Date</th>
                                <th class="border text-left" style="text-align: left;">Departure Date</th>
                        ...
                        <tbody>
                            @forelse ($parkings as $parking)
                                <tr>
                                    ...
                                    <td class="border">{{ $parking->arrival_date }}</td>
                                    <td class="border">{{ $parking->departure_date }}</td>
                                    <td class="border">{{ $parking->car_brand }}</td>
                                    <td class="border">{{ $parking->car_license_plate }}</td>
                                    <td class="border">
                                        <a href="{{ route('parkings.edit', $parking->id) }}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i> Edit</a> | 
                                        ...
                                        @endif
                                        <a href="{{ route('parkings.show', $parking->id) }}" class="btn btn-info"><i class="fa fa-eye" aria-hidden="true"></i> View</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
